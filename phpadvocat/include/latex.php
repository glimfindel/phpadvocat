...
function subst_letter_vars($templatefile, $targetfile, &$content, $pnumber, $address) {
  $returnval=0;
  $filecontent='';
  ...
  if(file_exists($templatefile)){ 
   ...
   $content = str_ireplace(chr(13), '', $filecontent);



  /* last write generated text into target file */
  if(strlen($targetfile) > 0) {
     if(($fd=fopen($targetfile, "w")) && (fwrite($fd, $content))) {
       fclose($fd);
       echo "written <hr>";
       $returnval=1;
     }
  } else { /* if no targetfile given, then don't use it */ 
       $returnval=1;
  }
