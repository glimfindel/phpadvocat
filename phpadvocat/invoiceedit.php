...
// Get invoice base number 
$dbcfg = new www_db;
$dbcfg->connect($user, $passwd);

$querystring="select invoice_base from phpa_config";
$dbcfg->query($querystring);
$dbcfg->next_record();
$invoice_base = $dbcfg->record['invoice_base'];
...
/* switch for computing the factor 1.0 charge */  
$updatecharge = false;
if ($_POST["getchargebutton"] <> NULL)
   $updatecharge = true;
...
if($_POST["invoiceeditbutton"]) {
      ...
      $service_start = toisodate($_POST["service_start"],$LOCALE);
      $service_end = toisodate($_POST["service_end"],$LOCALE);

      $pfile = $_POST["pfile"];
      $pfilevalue = toisonum($_POST["pfilevalue"],$LOCALE);
      $charge = toisonum($_POST["charge"],$LOCALE);
      $address = $_POST["address"];
      $object_services = $_POST["object_services"];
      $invoiceid = $_POST["invoiceid"];
      $invoicetext = $_POST["invoicetext"];
      
        $querystring = sprintf("update phpa_invoices set " .
                "createdate=%s, paydate=%s, service_start=%s, ".
                "service_end=%s, pfile=%s, address=%s, " .
                "pfilevalue=%s, charge=%s, object_services='%s', ".
                "invoiceid='%s', invoicetext='%s' " .
                " where number=%s",
                nullcorr($createdate), nullcorr($paydate),
                nullcorr($service_start), nullcorr($service_end),  
                $pfile, $address,
                nullcorr($pfilevalue), nullcorr($charge), $object_services, 
                $invoiceid, $invoicetext, $number);
	  // echo "<hr>".$querystring ."<hr>";
        if (!$db->query($querystring)) {
                $changecheck="Eintrag ge&auml;ndert";
            /* also compute the default charge */
                $updatecharge = true; 
...
// if($_POST["getchargebutton"]) { /* nor separate update Button any more since 0.8-8 */
      $number = $_POST["number"];
...
if($_POST["invposaddbutton"]) {
    ...
    $defaultchargefactor=0;  
      
    /* see if invoicetype has got a chargefactor */
    ...
    if($db->next_record())
            // $defaultchargefactor=toisonum($db->record["chargefactor"],$LOCALE);
            $defaultchargefactor=$db->record["chargefactor"];
    
    /* if nor value available, replace with 0 */
    if ($defaultchargefactor == NULL) 
            $defaultchargefactor=0;
    
    /* replace chargefactor with default if charegefactor is 0 */  
    if($chargefactor == 0) $chargefactor = $defaultchargefactor;
    ...
    if(($chargefactor == 0) && ($amount == "")) {
      ...
      if(($chargefactor > 0) && ($amount == 0)) {
      	/* get file value from invoice to calculate charges */
      ...
      if($amount == 0) { /* value neither got nor computed */
         /* do nothing but show that there's no value */
         $changecheck="Position ohne Wert";
      } else {
      	/* minimum amount is 10, so correct ist if it is less then 10 */ 
  ...
  $phpa_menue->selected = 3;
   array_insert($phpa_menue->contents,
      array( sprintf("&nbsp;&nbsp;<b><a href=pfileedit.php?pnumber=%s&detail=3>".
                     "Akte bearbeiten</a></b>",$pnumber)), 2);
   array_insert($phpa_menue->contents,
      array('&nbsp;&nbsp;&nbsp;&nbsp;<b>Rechnung</b>'), 3);
...
sprintf("<input name=pfilevalue type=text size=12 tabindex=1 value=%s align=right>\n".
       ...
       "<input name=getvaluebutton type=submit tabindex=12 ".
       "value=\"von Akte &uuml;bernehmen\" onClick=\"return confirm('Eintrag ueberschreiben?')\">\n",
         tolocalnum($db->record["pfilevalue"],$LOCALE)));


/* display only standard charge */
$invoicedialog->addinput("Geb&uuml;hr Faktor 1.0 nach RVG:",
sprintf("%s\n", tolocalnum($db->record["charge"],$LOCALE)));



/* input create date */
$invoicedialog->addinput("Rechnungsdatum:", 
sprintf("<input name=createdate type=text size=10 tabindex=2 value=\"%s\" ".
        "onDblClick=\"this.value=heute()\" >(TT.MM.JJJJ)\n",
        tolocaldate($db->record["createdate"],$LOCALE)));

/* input organizational invoice number */
$invoicedialog->addinput("interne Rechnungsnr.:", 
sprintf("<input name=invoiceid type=text size=10 tabindex=3 value=\"%s\"",
        $db->record["invoiceid"]));

/* input object of services / short description */
$invoicedialog->addinput("Leistungsgegenstand:", 
sprintf("<input name=object_services type=text size=60 tabindex=4 value=\"%s\"",
        $db->record["object_services"]));

/* input date of service begin */
$invoicedialog->addinput("Leistungszeitraum Beginn:", 
sprintf("<input name=service_start type=text size=10 tabindex=5 value=\"%s\" ".
        "onDblClick=\"this.value=heute()\" >(TT.MM.JJJJ)\n",
        tolocaldate($db->record["service_start"],$LOCALE)));

/* input date of service end */
$invoicedialog->addinput("Leistungszeitraum Ende:", 
sprintf("<input name=service_end type=text size=10 tabindex=6 value=\"%s\" ".
        "onDblClick=\"this.value=heute()\" >(TT.MM.JJJJ)\n",
        tolocaldate($db->record["service_end"],$LOCALE)));


/* input pay date */
$invoicedialog->addinput("Bezahlt am:", 
sprintf("<input name=paydate type=text size=10 tabindex=7 value=\"%s\" ".
        "onDblClick=\"this.value=heute()\" >(TT.MM.JJJJ)\n",
...
$optionlist = "<select name=address tabindex=8>\n";
...
$optionlist = "<select name=pfile tabindex=9>\n";
   while($dblist->next_record()) {
...
$invoicedialog->addinput("<input name=invoiceeditbutton type=submit ".
   "tabindex=10 value=Sichern>",
   "<input name=invoicepaybutton type=submit ".
   "tabindex=11 value=\"Rechnung bezahlen\" ".
...
echo "<hr>\n";
echo "<table width=100%><tr>";
printf ("<td align=center><a href=invoicepdf.php?number=%s&head=0 target=_BLANK>".
        "PDF-Ausgabe (ohne Kopf)</a></td>\n",$number);
printf ("<td align=center><a href=invoicepdf.php?number=%s&head=1 target=_BLANK>".
        "PDF-Ausgabe (mit Kopf)</a></td>\n",$number);
printf ("<td align=center><a href=invoicepdf.php?number=%s&head=2 target=_BLANK>".
        "PDF-Ausgabe (mit Standardkopf)</a></td>\n",$number);
echo "</tr></table>";
/* end framework */
echo "</TD></TR></TABLE>";
