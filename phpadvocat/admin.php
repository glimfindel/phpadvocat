...
if($_POST["admineditbutton"]) {
  ...
  $pdf_command=$_POST["pdf_command"];
  $invoice_base=$_POST["invoice_base"];
  ...
  $number=1;
   ...
   $querystring = sprintf("update phpa_config set " .
      ...
      "where number=%s",
       ...
       $filebase,
       pg_escape_string($pdf_command),
       nullcorr($invoice_base),
       $language,
       $number);
...
if($_POST["pwbutton"]) {
  ...
  $number=$_POST["number"];
   ...
   if ($passwd <> $oldpw) {
        $changecheck="Aktuelles Passwort nicht korrekt";
   } elseif ($newpw1 <> $newpw2) {
        $changecheck="Neue Passwoerter nicht identisch";
   } else {

/*
   	  if($DBSERVER=='POSTGRESQL') {
           $querystring = sprintf("alter user %s with password '%s' ", $user, $newpw1);
   	  } elseif($DBSERVER=='MYSQL') {
           $querystring = sprintf("SET PASSWORD = PASSWORD('%s')", $newpw1);
        }
        if (!$db->query($querystring)) {
*/
        if (!$db->setpassword($newpw1)) {
        	 $_SESSION["dbpasswd"]=$newpw1;
          $changecheck="Passwort ge&auml;ndert";
        } else {
        	 $changecheck="Passwort nicht ge&auml;ndert!";
        }
   }


}
/********** handle pw change *** end ********************************/
...
$admindialog->addinput("Kommando zum Erzeugen vom PDF <br>(%p f&uuml;r Dateiordner, %n f&uuml;r Dateiname, %f f&uuml;r gesamten Pfad):", 
   sprintf("<input name=pdf_command type=text size=60 value=\"%s\">\n",
        $db->record["pdf_command"]));
        
/* display invoice base */
$admindialog->addinput("Basisnummer f&uuml;r Rechnungen:", 
   sprintf("<input name=invoice_base type=text size=10 value=\"%s\">\n",
        $db->record["invoice_base"]));
...
/* beginning frame of dialog */
$pwdialog = new htmldialog;
 
/* only show client-nr, hidden detail nr */
$pwdialog->addinput("Passwort &auml;ndern f&uuml;r ", 
   sprintf("<input name=number type=hidden value=\"%s\"><b>%s</b>".
           "<input name=detail type=hidden value=\"%s\">",
           $db->record["number"],$user,$detail));

/* display title */
$pwdialog->addinput("Altes Passwort:", 
   "<input name=oldpw type=password size=15>\n");

$pwdialog->addinput("Neues Passwort:", 
   "<input name=newpw1 type=password size=15>\n");
$pwdialog->addinput("Noch mal:", 
   "<input name=newpw2 type=password size=15>\n");
$pwdialog->addinput("<input name=pwbutton type=submit value=Sichern>","");

$pwdialog->out(); /* End of display framework */
