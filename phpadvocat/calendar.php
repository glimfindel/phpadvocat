...
// get events
  ...
  $db->query($querystring);
   ...
   // get deadlines
   $querystring = sprintf("select phpa_deadlines.eventday, ".
       "phpa_deadlines.description, phpa_deadlines.type, ".
       "phpa_deadlines.pfile, phpa_pfiles.processregister ".
       "from phpa_deadlines, phpa_pfiles ".
       "where phpa_deadlines.pfile = phpa_pfiles.number ".
       "order by eventday");

   $db->query($querystring);

   /* Display all deadlines today */
   $deadlines=array();
   $predeadlines=array();
   while($db->next_record()) {
       $eventstarttime=
            substr($db->record["eventday"], 0,4).
            substr($db->record["eventday"], 5,2).
            substr($db->record["eventday"], 8,2);
       $eventtext=
           "<a href=\"pfileedit.php?pnumber=" . $db->record["pfile"] . "#detail\">&bull;&nbsp;" . 
           $db->record["description"].'&nbsp;('.
           $db->record["processregister"].')</a><br />';
       if($db->record["type"] == 0) {
           $predeadlines[$eventstarttime] .= $eventtext;
       } else {
           $deadlines[$eventstarttime] .= $eventtext;
       }
   } /* end while */
...
$params['predeadlines'] = $predeadlines;
  ...
  $db->query($querystring);
   ...
   while($db->next_record()) {
	...
	foreach(explode(",", $date) as $day) {
		...
		$params['time_start'] = '6:00';
		$params['time_stop'] = '23:30';
		$params['highlight'] = $eventarray;
