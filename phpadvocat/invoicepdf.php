...
if((!$db->query($querystring)) && $db->next_record()) { 
  ...
  $object_services=$db->record["object_services"];
  $invoiceid=$db->record["invoiceid"];
  $createdate=tolocaldate($db->record["createdate"],$LOCALE);
  $service_start=tolocaldate($db->record["service_start"],$LOCALE);
  $service_end=tolocaldate($db->record["service_end"],$LOCALE);
...
$pdf->AddPage(); /* ***************** new Page with invoice ************************* */
...
$pdf->Output("Rechnung-".$invoiceid.".pdf","I");
// $pdf->Output();
?>
