...
if ($loginuser && $loginpasswd) {
   ...
   if ($db->conn == false){ /* Wrong connect 0 => return to login! */
...
print ("<h2>N&auml;chste Fristen</h2>\n");
     ...
     $querystring = sprintf("select phpa_deadlines.eventday, ".
          ...
          "where date(eventday) >= (date 'today') ".
          "and phpa_deadlines.pfile = phpa_pfiles.number ".
          "order by eventday limit 5");
     } elseif ($DBSERVER=='MYSQL'){     
     $querystring = sprintf("select phpa_deadlines.eventday, ".
          "phpa_deadlines.description, phpa_deadlines.type, ".
          "phpa_deadlines.pfile, phpa_pfiles.processregister ".
          "from phpa_deadlines, phpa_pfiles ".
          "where DATE_FORMAT(eventday,'%%Y-%%m-%%d') >= CURDATE() ".
          "and phpa_deadlines.pfile = phpa_pfiles.number ".
          "order by eventday limit 5");
     }
